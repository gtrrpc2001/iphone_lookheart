...
class SettingProfile: UIView {
    ...
    /*------------------------- Dictionary ------------------------*/
    // region
    private lazy var backgroundMapping: [NotificationManager.NotiType : UILabel] = [:]
    private lazy var buttonMapping: [Int : UIButton] = [:]
    
    private lazy var tagMapping: [NotificationManager.NotiType : Int] = [:]
    // endregion
    ...
    public func updateProfile() {
        for type in backgroundMapping.keys {
            if type != .null {
                let enabled = NotificationManager.shared.getNotificationEnabled(type: type)
                setOnOffButtonColor(tagMapping[type]! - 1, tagMapping[type]!, enabled)
            }
        }
    }
    ...
    private func logoutEvent() {
        ...
        // init Singleton
        HealthDataManager.shared.resetHealthData()
        HourlyDataManager.shared.resetHourlyData()
    ...
    // MARK: -
    private func createButtons(type: NotificationManager.NotiType, background: UILabel) {
    ...
    private func createButton(type: NotificationManager.NotiType, enableFlag: Bool) -> UIButton {
        
        let notiEnabled = NotificationManager.shared.getNotificationEnabled(type: type)
        ...
        let button = UIButton().then {
            ...
            $0.setTitleColor(setButtonTextColor(notiEnabled, enableFlag), for: .normal)
            $0.backgroundColor = setButtonColor(notiEnabled, enableFlag)
    ...
    @objc private func buttonEvent(_ sender: UIButton) {
        ...
        switch tag {
        case EMERGENCY_ENABLE, EMERGENCY_DISABLE:
            enable = tag == EMERGENCY_ENABLE ? true : false
            notificationManager.setNotificationEnabled(type: .emergency, flag: enable)
            setOnOffButtonColor(EMERGENCY_ENABLE, EMERGENCY_DISABLE, enable)
            
        case ARR_ENABLE, ARR_DISABLE:
            enable = tag == ARR_ENABLE ? true : false
            notificationManager.setNotificationEnabled(type: .arr, flag: enable)
            setOnOffButtonColor(ARR_ENABLE, ARR_DISABLE, enable)
            
        case FAST_ENABLE, FAST_DISABLE:
            enable = tag == FAST_ENABLE ? true : false
            notificationManager.setNotificationEnabled(type: .fast, flag: enable)
            setOnOffButtonColor(FAST_ENABLE, FAST_DISABLE, enable)
            
        case SLOW_ENABLE, SLOW_DISABLE:
            enable = tag == SLOW_ENABLE ? true : false
            notificationManager.setNotificationEnabled(type: .slow, flag: enable)
            setOnOffButtonColor(SLOW_ENABLE, SLOW_DISABLE, enable)
            
        case HEAVY_ENABLE, HEAVY_DISABLE:
            enable = tag == HEAVY_ENABLE ? true : false
            notificationManager.setNotificationEnabled(type: .irregular, flag: enable)
            setOnOffButtonColor(HEAVY_ENABLE, HEAVY_DISABLE, enable)
            
        case MYO_ENABLE, MYO_DISABLE:
            enable = tag == MYO_ENABLE ? true : false
            notificationManager.setNotificationEnabled(type: .myo, flag: enable)
            setOnOffButtonColor(MYO_ENABLE, MYO_DISABLE, enable)
            
        case NONCONTACT_ENABLE, NONCONTACT_DISABLE:
            enable = tag == NONCONTACT_ENABLE ? true : false
            notificationManager.setNotificationEnabled(type: .noncontact, flag: enable)
            setOnOffButtonColor(NONCONTACT_ENABLE, NONCONTACT_DISABLE, enable)
             
        case HOURLY_ARR_ENABLE, HOURLY_ARR_DISABLE:
            enable = tag == HOURLY_ARR_ENABLE ? true : false
            notificationManager.setNotificationEnabled(type: .hourly, flag: enable)
            setOnOffButtonColor(HOURLY_ARR_ENABLE, HOURLY_ARR_DISABLE, enable)
            
        case TOTAL_ARR_ENABLE, TOTAL_ARR_DISABLE:
            enable = tag == TOTAL_ARR_ENABLE ? true : false
            notificationManager.setNotificationEnabled(type: .total, flag: enable)
    ...
    private func setButtonTag(_ type: NotificationManager.NotiType, _ flag: Bool) -> Int {
        switch type {
        case .emergency:
    ...
    // MARK: -
    private func createBackgroundLabel(type: NotificationManager.NotiType) -> UILabel {
    ...
    // MARK: -
    private func createLabel(type: NotificationManager.NotiType) -> UILabel {
        let label = UILabel().then {
    ...
    private func setLabelText(type: NotificationManager.NotiType) -> String {
        switch type {
        case .emergency:
    ...
    private func addView() {
        ...
        let heavyLabelSV = UIStackView().then {
            $0.addArrangedSubview(createLabel(type: .irregular))
        ...
        let heavyBackgroundSV = UIStackView().then {
            $0.addArrangedSubview(createBackgroundLabel(type: .irregular))
