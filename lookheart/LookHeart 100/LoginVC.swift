...
class LoginVC : TitleViewController, UITextFieldDelegate, DataPassingDelegate {
    ...
    @objc func loginButtonEvent(_ sender: UIButton) {
        loginServerTask(idInput, pwInput)
    }
    ...
    private func loginServerTask(_ id: String, _ pw: String) {
        NetworkManager.shared.checkLoginToServer(id: id, pw: pw, destroy: false) { result in
            ...
            case .success(let isAvailable):
                ...
                case .successLogin:
                    self.successLogin()
                case .failureLogin:
                    propAlert.basicAlert(title: "loginFailed".localized(), message: "incorrectlyLogin", ok: "ok".localized(), viewController: self)
                case .duplicateLogin:
                    propAlert.basicAlert(title: "loginFailed".localized(), message: "duplicateLogin", ok: "ok".localized(), viewController: self)
                }
