...
class BluetoothManager: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        print("Disconnected!")
        NetworkManager.shared.sendBleLog(action: .BleDisconnect)
    ...
    private func stepReceived(step: Int) {
        ...
        if stepCal >= NotificationThreshold {
            handleNotification(for: stepCal / 10)
        }
    ...
    private func handleNotification(for notificationType: Int) {
        ...
        case NONCONTACT_TAG:
            NotificationManager.shared.sendNonContactNoti()
        case MYO_TAG:
            NotificationManager.shared.sendMyoNoti()
        case ARR_TAG:
            arrFlag = notificationType
        case FAST_ARR_TAG:
        ...
        if arrFlag != 0 && ecgData.count == 500 {
            sendArrData(arrFlag: arrFlag)
        }
    }
    ...
    private func handleDateChange() {
        ...
        NotificationManager.shared.resetToalArray()
    ...
    private func sendArrData(arrFlag: Int) {
        
        let currentDateTime = "\(currentDate) \(currentTime)"
        let bodyState = healthDataManager.bodyState
        let arrStatus = ArrState[arrFlag] ?? ARR_STATE
        let arrArray = ecgData.map { String($0) }
        let arrEcgData = arrArray.joined(separator: ",")
        
        let arrData = "\(currentTime),\(propTimeZone),\(bodyState),\(arrStatus),\(arrEcgData)"
        ...
        NetworkManager.shared.sendArrDataToServer(arrData: arrParams) { [self] result in
            ...
            case .success(let isSuccess):
                ...
                if isSuccess {
                    ...
                    NotificationManager.shared.sendArrNoti(for: arrFlag)
