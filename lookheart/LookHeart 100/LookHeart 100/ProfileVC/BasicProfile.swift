...
class BasicProfile: UIView, UITextFieldDelegate {
    ...
    private let NAME_TAG = 1
    private let PHONE_TAG = 2
    private let HEIGHT_TAG = 3
    private let WEIGHT_TAG = 4
    private let SLEEP_TAG = 5
    private let WAKEUP_TAG = 6
    ...
    private lazy var textFields: [UITextField] = [nameTextField, phoneTextField, heightTextField, weightTextField]
    ...
    /*------------------------- Background ------------------------*/
    private let nameBackground = UILabel().then {
    ...
    func setProfile(_ key: String, _ value: String){
        ...
        case "Name":
            ...
            UserProfileManager.shared.name = value
        case "Birthday":
    ...
    public func updateProfile() {
        let gender = UserProfileManager.shared.gender == "남자" ? "male_Label".localized() : "female_Label".localized()
        
        let bedTime = String(UserProfileManager.shared.bedTime)
        let wakeupTime = String(UserProfileManager.shared.wakeUpTime)
        
        let phone = UserProfileManager.shared.phone
        let phoneText = phone == "01012345678" ? "phoneHintText2".localized() : phone
        
        age.text = UserProfileManager.shared.age
        birthday.text = UserProfileManager.shared.birthDate
        genderButton.setTitle(gender, for: .normal)
        
        nameTextField.attributedPlaceholder = NSAttributedString(string: UserProfileManager.shared.name, attributes: attributes)
        phoneTextField.attributedPlaceholder = NSAttributedString(string: phoneText, attributes: attributes)
        heightTextField.attributedPlaceholder = NSAttributedString(string: UserProfileManager.shared.height, attributes: attributes)
        weightTextField.attributedPlaceholder = NSAttributedString(string: UserProfileManager.shared.weight, attributes: attributes)
        sleepTextField.attributedPlaceholder = NSAttributedString(string: bedTime, attributes: attributes)
        wakeupTextField.attributedPlaceholder = NSAttributedString(string: wakeupTime, attributes: attributes)
    }
    ...
    private func addView() {
        ...
        nameBackground.snp.makeConstraints { make in
        ...
        nameTextField.snp.makeConstraints { make in
            make.left.equalTo(nameBackground).offset(5)
            make.top.right.bottom.equalTo(nameBackground)
        }
        ...
        phoneBackground.snp.makeConstraints { make in
            ...
            make.left.right.equalTo(nameBackground)
            make.height.equalTo(35)
        }
